"""
Scenario 9 Training/Evaluation: Unanimous agreement (3/3 surgeons), CT + demographics.

Run:
  python 04h_train_evaluate_unanimous_with_demographics.py
"""
...
RESULTS_DIR = "results_unanimous_with_demographics_same_split"
...
def plot_outputs(results, y_test):
    ...
    ax.set_title("Scenario 9 ROC Curves")
    ...
    ax.set_title("Scenario 9 PR Curves")
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Scenario 9 Metric Comparison")
    ax.legend(loc="lower right", fontsize=9)
    plt.tight_layout()
...
def save_results(cv_res, test_res, cohort_res, meta):
    ...
    payload = {
        "analysis_variant": meta.get("analysis_variant", "scenario9_unanimous_ct_plus_demo"),
    ...
    with open(os.path.join(RESULTS_DIR, "results_summary.txt"), "w") as f:
        f.write("Scenario 9 (Unanimous 3/3 agreement, CT+demographics) Results\n")
        f.write("=" * 63 + "\n\n")
...
def main():
    ...
    print(f"Saved scenario 9 outputs to {RESULTS_DIR}")
