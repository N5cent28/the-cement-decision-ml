"""
Scenario 5 Training/Evaluation: Halldor+3D agreement only, CT features only.

Run:
  python 04e_train_evaluate_agree_ct_only.py
"""
...
RESULTS_DIR = "results_scenario5_agree_ct_only"
...
def plot_outputs(results, y_test):
    ...
    ax.set_title("Scenario 5 ROC Curves")
    ...
    ax.set_title("Scenario 5 PR Curves")
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Scenario 5 Metric Comparison")
    ax.legend(loc="lower right", fontsize=9)
    plt.tight_layout()
...
def save_results(cv_res, test_res, cohort_res, meta):
    ...
    payload = {
        "analysis_variant": meta.get("analysis_variant", "scenario5_h3d_agreement_ct_only"),
    ...
    with open(os.path.join(RESULTS_DIR, "results_summary.txt"), "w") as f:
        f.write("Scenario 5 (Halldor+3D agree, CT-only) Results\n")
        f.write("=" * 52 + "\n\n")
...
def main():
    ...
    print(f"Saved scenario 5 outputs to {RESULTS_DIR}")
