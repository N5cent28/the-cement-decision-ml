"""
Scenario 8 Training/Evaluation: Unanimous agreement (3/3 surgeons), demographics-only.

Run:
  python 04i_train_evaluate_unanimous_demographics_only.py
"""
...
RESULTS_DIR = "results_unanimous_demographics_only_same_split"
...
def plot_outputs(results, y_test):
    ...
    ax.set_title("Scenario 8 ROC Curves")
    ...
    ax.set_title("Scenario 8 PR Curves")
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Scenario 8 Metric Comparison")
    ax.legend(loc="lower right", fontsize=9)
    plt.tight_layout()
...
def save_results(cv_res, test_res, cohort_res, meta):
    ...
    payload = {
        "analysis_variant": meta.get("analysis_variant", "scenario8_unanimous_demographics_only"),
    ...
    with open(os.path.join(RESULTS_DIR, "results_summary.txt"), "w") as f:
        f.write("Scenario 8 (Unanimous 3/3 agreement, demographics-only) Results\n")
        f.write("=" * 65 + "\n\n")
...
def main():
    ...
    print(f"Saved scenario 8 outputs to {RESULTS_DIR}")
