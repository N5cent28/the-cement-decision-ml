"""
Repeated grouped/stratified split evaluation for scenarios 1, 3, 4, 5, and 6.

Purpose:
  Reduce dependence on one accidental test split by running repeated
  patient-grouped train/test splits and summarizing metric distributions.

Run examples:
  python 07_repeated_grouped_split_eval.py
  python 07_repeated_grouped_split_eval.py --repeats 20 --test-size 0.2
"""
...
OUT_DIR = "results_repeated_splits_scenarios_1_3_4_5_6"
...
class ScenarioDef:
    ...
    use_ct: bool
    use_demo: bool
    agreement_only_h3d: bool = False


SCENARIOS = [
    ScenarioDef("scenario1_majority_ct_only", "gt_majority", use_ct=True, use_demo=False),
    ScenarioDef("scenario3_majority_ct_plus_demo", "gt_majority", use_ct=True, use_demo=True),
    ScenarioDef("scenario4_majority_demo_only", "gt_majority", use_ct=False, use_demo=True),
    ScenarioDef("scenario5_h3d_agree_ct_only", "gt_h3d_agree", use_ct=True, use_demo=False, agreement_only_h3d=True),
    ScenarioDef("scenario6_original_ct_plus_demo", "gt_original", use_ct=True, use_demo=True),
]
...
def load_clean_base():
    ...
    # Match cleaning logic used in prior analyses.
...
def stratified_group_split(df, target_col, test_size, seed):
    patient_info = (
        df.groupby("Anonymize_ID")
        .agg({target_col: "first", "Cohort_group": "first"})
        .reset_index()
    )
    # Prefer target+cohort stratification.
    patient_info["strat_tc"] = patient_info[target_col].astype(int).astype(str) + "_" + patient_info["Cohort_group"]
    ...
    try:
        tr, te = train_test_split(
            ...
            test_size=test_size,
            random_state=seed,
            stratify=patient_info["strat_tc"],
        )
    except ValueError:
        # Fallback if sparse strata.
...
def run_single_repeat(df_base, scenario, seed, test_size):
    df = df_base.copy()
    if scenario.agreement_only_h3d:
        df = df[df["gt_h3d_agree"].notna()].copy()
    else:
        df = df[df[scenario.target_col].notna()].copy()

    train, test, split_strategy = stratified_group_split(df, scenario.target_col, test_size=test_size, seed=seed)
...
def summarize_metrics(df):
    summary = (
        ...
        .agg(
            ...
            roc_auc_p05=("roc_auc", lambda s: s.quantile(0.05)),
            roc_auc_p95=("roc_auc", lambda s: s.quantile(0.95)),
        ...
        .sort_values(["scenario", "roc_auc_median"], ascending=[True, False])
    )
    return summary
...
def plot_roc_auc_distributions(df):
    ...
    for scenario in sorted(df["scenario"].unique()):
        ...
        ax.set_title(f"{scenario}: ROC-AUC distribution across repeated splits")
        ax.set_xlabel("")
        ax.set_ylabel("ROC-AUC")
        ax.tick_params(axis="x", rotation=20)
        plt.tight_layout()
        fig.savefig(os.path.join(FIG_DIR, f"{scenario}_roc_auc_boxplot.png"), dpi=160, bbox_inches="tight")
        plt.close(fig)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--repeats", type=int, default=10, help="Number of repeated splits per scenario.")
    ...
    split_usage_df = (
        raw_df.groupby(["scenario", "split_stratification_mode"], as_index=False)
        .size()
        .rename(columns={"size": "model_rows"})
        .sort_values(["scenario", "model_rows"], ascending=[True, False])
    )
    ...
    print(f"Saved repeated-split results to {OUT_DIR}")
