"""
Scenario 12 Training/Evaluation: Vote fraction (continuous), demographics-only.

Run:  python 04m_train_evaluate_vote_fraction_demographics_only.py
"""
...
PROCESSED_DIR = "processed_data_vote_fraction_demographics_only_same_split"
RESULTS_DIR = "results_vote_fraction_demographics_only_same_split"
...
def plot_scatter_by_model(results, y_test):
    ...
    plt.suptitle("Scenario 12: Predicted vs True (Demographics-Only)", y=1.02)
    plt.tight_layout()
...
def plot_regression_metrics(results):
    ...
    ax.set_title("Scenario 12: Model Comparison (Demographics-Only)")
...
def plot_binary_view(results, y_test):
    ...
    for ax, (name, res) in zip(axes, results.items()):
        ...
        ax.set_ylabel("True")
    plt.suptitle("Scenario 12: Thresholded Binary View (0.5)", y=1.02)
    plt.tight_layout()
...
def save_results(cv_results, test_results, cohort_results, meta):
    ...
    data = {
        "analysis_variant": meta.get("analysis_variant", "scenario12_vote_fraction_demographics_only"),
    ...
    with open(os.path.join(RESULTS_DIR, "results_summary.txt"), "w") as f:
        f.write("Scenario 12 (Vote fraction, demographics-only) Results\n")
        f.write("=" * 55 + "\n\n")
...
def main():
    ...
    print(f"Saved scenario 12 outputs to {RESULTS_DIR}")
