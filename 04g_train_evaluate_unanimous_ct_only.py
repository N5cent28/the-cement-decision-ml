"""
Scenario 7 Training/Evaluation: Unanimous agreement (3/3 surgeons), CT-only.

Run:
  python 04g_train_evaluate_unanimous_ct_only.py
"""
...
RESULTS_DIR = "results_unanimous_ct_only"
...
def plot_outputs(results, y_test):
    ...
    ax.set_title("Scenario 7 ROC Curves")
    ...
    ax.set_title("Scenario 7 PR Curves")
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Scenario 7 Metric Comparison")
    ax.legend(loc="lower right", fontsize=9)
    plt.tight_layout()
...
def save_results(cv_res, test_res, cohort_res, meta):
    ...
    payload = {
        "analysis_variant": meta.get("analysis_variant", "scenario7_unanimous_ct_only"),
    ...
    with open(os.path.join(RESULTS_DIR, "results_summary.txt"), "w") as f:
        f.write("Scenario 7 (Unanimous 3/3 agreement, CT-only) Results\n")
        f.write("=" * 56 + "\n\n")
...
def main():
    ...
    print(f"Saved scenario 7 outputs to {RESULTS_DIR}")
