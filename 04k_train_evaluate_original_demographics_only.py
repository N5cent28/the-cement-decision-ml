"""
Scenario 11 Training/Evaluation: Original operating surgeon's decision, demographics-only.

Run:
  python 04k_train_evaluate_original_demographics_only.py
"""
...
RESULTS_DIR = "results_original_demographics_only_same_split"
...
def plot_outputs(results, y_test):
    ...
    ax.set_title("Scenario 11 ROC Curves")
    ...
    ax.set_title("Scenario 11 PR Curves")
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Scenario 11 Metric Comparison")
    ax.legend(loc="lower right", fontsize=9)
    plt.tight_layout()
...
def save_results(cv_res, test_res, cohort_res, meta):
    ...
    payload = {
        "analysis_variant": meta.get("analysis_variant", "scenario11_original_demographics_only"),
    ...
    with open(os.path.join(RESULTS_DIR, "results_summary.txt"), "w") as f:
        f.write("Scenario 11 (Original surgeon target, demographics-only) Results\n")
        f.write("=" * 66 + "\n\n")
...
def main():
    ...
    print(f"Saved scenario 11 outputs to {RESULTS_DIR}")
