"""
Scenario 6 Training/Evaluation: Original surgeon target, CT + demographics.

Run:
  python 04f_train_evaluate_original_ct_plus_demo.py
"""
...
RESULTS_DIR = "results_scenario6_original_ct_plus_demo"
...
def plot_outputs(results, y_test):
    ...
    ax.set_title("Scenario 6 ROC Curves")
    ...
    ax.set_title("Scenario 6 PR Curves")
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Scenario 6 Metric Comparison")
    ax.legend(loc="lower right", fontsize=9)
    plt.tight_layout()
...
def save_results(cv_res, test_res, cohort_res, meta):
    ...
    payload = {
        "analysis_variant": meta.get("analysis_variant", "scenario6_original_ct_plus_demo"),
    ...
    with open(os.path.join(RESULTS_DIR, "results_summary.txt"), "w") as f:
        f.write("Scenario 6 (Original surgeon target, CT+demographics) Results\n")
        f.write("=" * 62 + "\n\n")
...
def main():
    ...
    print(f"Saved scenario 6 outputs to {RESULTS_DIR}")
