"""
Step 4D: Model Training & Evaluation (Majority + Demographics-Only)
====================================================================
Trains the same model family using only demographic features on the exact
original majority-vote split.

Input:
  processed_data_demographics_only_same_split/

Output:
  results_majority_demographics_only_same_split/

Run: python 04d_train_evaluate_majority_demographics_only.py
"""
...
RESULTS_DIR = "results_majority_demographics_only_same_split"
...
def cross_validate_models(models, X, y, groups):
    print("=" * 72)
    print("GROUPED CROSS-VALIDATION (majority + demographics-only)")
    print("=" * 72)
...
def evaluate_on_test(models, X_train, y_train, X_test, y_test):
    print("=" * 72)
    print("TEST SET EVALUATION (majority + demographics-only)")
    print("=" * 72)
    out = {}
    for name, model in models.items():
        ...
        out[name] = {"metrics": metrics, "y_pred": y_pred, "y_proba": y_proba}
        print(f"\n  {name}:")
...
def plot_roc(results, y_test):
    ...
    ax.set_title("ROC Curves — Majority + Demographics-Only")
...
def plot_pr(results, y_test):
    ...
    ax.set_title("PR Curves — Majority + Demographics-Only")
...
def plot_confusions(results, y_test):
    ...
    for ax, (name, res) in zip(axes, results.items()):
        ...
        ax.set_ylabel("True")
    plt.suptitle("Confusion Matrices — Majority + Demographics-Only", y=1.02)
    plt.tight_layout()
...
def plot_metric_bars(results):
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Model Metrics — Majority + Demographics-Only")
    ax.legend(loc="lower right", fontsize=9)
    plt.tight_layout()
...
def save_results(cv_results, test_results, cohort_results, meta):
    ...
    payload = {
        ...
        "preprocessing": {
            ...
            "n_ct_features": len(meta.get("ct_feature_columns", [])),
            "split_stratification_mode": meta.get("split_stratification_mode", "from_base_split_or_not_recorded"),
        },
    }
    ...
    with open(summary_path, "w") as f:
        f.write("THA Cement Decision — Majority + Demographics-Only Summary\n")
        f.write("=" * 62 + "\n\n")
...
def main():
    print("=" * 72)
    print("STEP 4D: MAJORITY-VOTE TRAINING WITH DEMOGRAPHICS-ONLY FEATURES")
    print("=" * 72 + "\n")
    ...
    print("=" * 72)
    print("TRAINING COMPLETE (MAJORITY + DEMOGRAPHICS-ONLY)")
    print("=" * 72)
