...
Ground truth here is the TRUE 3-rater unanimous consensus (original surgeon +
Halldor + 3D all agree), which is distinct from `gt_h3d_agree` used in
scenario 5 (that target only requires the Halldor and 3D raters to agree,
ignoring the original surgeon's label).

This script establishes the base patient-grouped split for the "unanimous"
ground-truth family; scenarios 8 (demo-only) and 9 (CT+demo) reuse this same
split so the three feature-set variants differ by exactly one variable.
...
def build_targets(df):
    """True 3-rater unanimous consensus: all three surgeons must agree."""
...
def main():
    ...
    train, test, split_strategy = grouped_split(df, target_col="gt_unanimous")
